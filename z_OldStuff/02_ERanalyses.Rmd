...
```{r echo=FALSE}
ggplot(met, aes(y = log(lER), x = doy, color = treatment)) +
  geom_point() +
  geom_line() +
  facet_grid(. ~ stream)
```
...
I'm seeing some weird relationships between lER residuals and Q during some years.
...
metS6 <- met %>% 
  filter(stream == "st6") %>% 
  select(doy, Tanom.iktCs, meanQds.lcs)
  

phys.pca <- prcomp(metS6, center = TRUE, scale. = TRUE)
library(devtools)
...
all weird residuals all now with S18P
```{r}
met2 <- met %>% 
  filter(doy >= 180) %>% 
...
ggplot(met2, aes(y = DD, x = doy)) +
  geom_point() +
  facet_wrap(vars(stream, treatment), ncol = 3)

g.er.global1 <- gamm(lER ~ treatment+invKT.C.StMean +  s(Qres, by = stream),
                     REML = FALSE, data = met2)
g.er.global2 <- gamm(lER ~ treatment+invKT.C.StMean +  s(DD, by = stream),
                     REML = FALSE, data = met2)
g.er.global3 <- gamm(lER ~ treatment+invKT.C.StMean +  s(DD, by = treatment),
                     REML = FALSE, data = met2)
g.er.global4 <- gamm(lER ~ treatment+invKT.C.StMean +  s(Qres, by = stream) + s(DD, by = stream),
                     REML = FALSE, data = met2,
                     control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
g.er.global4b <- gamm(lER ~ treatment+invKT.C.StMean +  s(Qres) + s(DD, by = stream),
                     REML = FALSE, data = met2)
g.er.global5 <- gamm(lER ~ treatment+invKT.C.StMean +  s(Qres, by = stream) + s(DD, by = treatment),
                     REML = FALSE, data = met2)
g.er.global6 <- gamm(lER ~ treatment*invKT.C.StMean + treatment*DD + s(Qres, by = stream),
                     REML = FALSE, data = met2)
g.er.global7 <- gamm(lER ~ treatment*invKT.C.StMean + treatment+DD + s(Qres, by = stream),
                     REML = FALSE, data = met2)



model.sel(g.er.global1$lme, g.er.global2$lme, g.er.global3$lme, g.er.global5$lme, g.er.global6$lme, g.er.global7$lme, g.er.global4b$lme)

summary(g.er.global4b$gam)
summary(g.er.global6$gam)
